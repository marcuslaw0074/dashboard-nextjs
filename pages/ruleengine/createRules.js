...
class BasicTable extends React.Component {
  ...
  render() {
    return (
      <>
        <Link href="/ruleengine/dashboards">Dashboard</Link>
        <AddExpressionRow
          data={{
            expressions: [],
            expression: "",
...
// 
// ==> transform to --golang
// FuncMaps := [][map[string]string][map[string]string{"ineq": <inequality>, "exp": <inequality>}] 
// 
// ==> transform to --golang
...
// }
// 
// func (fMap []map[string]string) (func (...float64) float64, error) {
// allFunction := make([]*ConditionFunctions, 0)
// for ind, mapping := range fMap {
//   ex := &goparser.Function{}
//   ex.GenerateFunctions("a+(b+v)*a+a/b/a/a", fmt.Sprintf("EXP_%v", ind))
//   inq, _ := goparser.InputExpression("((f>a+(b+v)*a+a/b/a/a) and (g>b+v)) or (u>b+v*o)")
//   allFunction = append(allFunction, &ConditionFunctions{
//    ExpressionFunction: ex.CallFunctionByMap,
//    ExpressionMapping: ex.Mapping,
//    InequalityFunction: inq.CallFunctionByMap,
